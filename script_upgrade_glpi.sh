...
if [ -d "/var/www/glpi" ]; then
    echo -e "\E[31mErreur : le répertoire /var/www/glpi existe."
    exit 1
fi
...
if [ "$reponse" = "o" ]; then
    ...
    echo
    read -s password
    mysqldump -u root -p$password --databases "$database_name" > "/tmp/backup_${database_name}.sql" && echo "La sauvegarde de la base de données '$database_name' a été créée avec succès dans /tmp." || { echo -e "\E[31mErreur : échec de la sauvegarde de la base de données."; exit 1; }

    # Sauvegarde du répertoire plugins
    echo "Sauvegarde du répertoire plugins dans /tmp..."
    mv /var/www/html/glpi/plugins /tmp/ && echo "Sauvegarde du répertoire plugins réussie !" || { echo -e "\E[31mErreur : échec de la sauvegarde du répertoire plugins."; exit 1; }

    # Sauvegarde du répertoire marketplace
    echo "Sauvegarde du répertoire marketplace dans /tmp..."
    mv /var/www/html/glpi/marketplace /tmp/ && echo "Sauvegarde du répertoire marketplace réussie !" || { echo -e "\E[31mErreur : échec de la sauvegarde du répertoire marketplace."; exit 1; }

    # Sauvegarde du fichier downstream
    echo "Sauvegarde du fichier downstream.php dans /tmp..."
    mv /var/www/html/glpi/inc/downstream.php /tmp/ && echo "Sauvegarde du fichier downstream.php réussie !" || { echo -e "\E[31mErreur : échec de la sauvegarde du fichier downstream.php."; exit 1; }
    ...
    wget https://github.com/glpi-project/glpi/releases/download/${version}/glpi-${version}.tgz && echo "Téléchargement réussi !" || { echo -e "\E[31mErreur : échec du téléchargement."; exit 1; }
    echo "Décompression du répertoire GLPI..."
    tar xvzf glpi-${version}.tgz && echo "Décompression réussie !" || { echo -e "\E[31mErreur : échec de la décompression."; exit 1; }
    ...
    cp /tmp/downstream.php /var/www/html/glpi/inc/ && echo "Copie du fichier downstream.php réussi !" || { echo -e "\E[31mErreur : échec de la copie du fichier downstream.php."; exit 1; }
